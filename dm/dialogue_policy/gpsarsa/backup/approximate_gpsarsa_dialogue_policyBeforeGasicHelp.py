...
class ApproximateGPSarsaDialoguePolicy(DialoguePolicy):
    ...
    def __init__(self):
        ...
        self.sigma = 5.916#square root of the interval betwen max and min possible reward, using in update Q.
        self.epsilon = 0.2#the probability of take randomize action for exploration
        ...
        self.threshold_v = 0.1# threshold to push new data point to approximate dictionary D.
        self.means = None
        self.C= None
        self.c = None
        self.v1 = 0
    ...
    def _read(self, fin):
        ...
        #read matrix/vector/tuple B, K, H, R, Q, D etc from file
...
##            self.update_q()
    ...
    def _update_K1(self):
        ...
        self.K1 = self.K1 + self.g*self.g.T
    ...
    def _calculate_from_second_turn(self, end_dialogue = False):
        ...
        if self.delta>self.threshold_v and not end_dialogue:
            ...
            self.delta_ktt = np.dot(self.g_comma.T,(self.last_cov_ba-2*self.gamma*self.cov_ba)) + pow(self.gamma, 2)*self.kernel((self.b, self.a),(self.b, self.a)) #line 30
            if self.v1 == 0:#line 31+32
                ...
                self.v1 = (1 + pow(self.gamma,2))*pow(self.sigma, 2) + self.delta_ktt - np.dot(self.delta_k.T, self.C).dot(self.delta_k) + (2*self.gamma*pow(self.sigma,2)/self.v1)*np.dot(self.c.T, self.delta_k) - pow(self.gamma, 2)*pow(self.sigma, 4)/self.v1#line 32
            ...
            #Solution: reset self.c fix with new D
            #self.c = np.vstack((self.c, 0))
            ...
            if not end_dialogue:#can be wrote shorter, but it is not neassary
                if self.v1==0:#line 37
                    ...
                    self.v1 = (1 + pow(self.gamma,2))*pow(self.sigma, 2) + np.dot(self.delta_k.T, (self.c_comma + (self.gamma*pow(self.sigma, 2)/self.v1)*self.c)) - pow(self.gamma, 2)*pow(self.sigma, 4)/self.v1
            else:#terminal turn
        ...
        #pdb.set_trace()
        #Problem:CHANGE: CHECK self.c -> self.c_comma
    ...
    def get_da(self, user_act_queue, belief_tracker):
        ...
        self.b = self.basic_fun(belief_tracker.get_belief_space())
    ...
    def calculate_q(self, b, a):
        ...
        if variance <0:
            ...
            pdb.set_trace()
        return np.random.normal(mean, pow(variance,0.5))
        #return mean
